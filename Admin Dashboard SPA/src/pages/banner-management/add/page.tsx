...
export default function AddBannerPage() {
  ...
  useEffect(() => {
    const validCityIds = availableCities.map((c) => c.id)
    setSelectedCities((prev) => prev.filter((c) => validCityIds.includes(c)))
  }, [selectedStates])

  // Reset pincodes when cities change
  useEffect(() => {
    const validPincodeIds = availablePincodes.map((p) => p.id)
    setSelectedPincodes((prev) => prev.filter((p) => validPincodeIds.includes(p)))
  }, [selectedCities])
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Dashboard - Advertisement / Banner Management - Add Banner</p>
        </div>
                  ...
                  >
                    {(["Top", "Bottom", "Left", "Right"] as PositionType[]).map((position) => (
                      <div key={position} className="flex items-center space-x-2">
                        <RadioGroupItem value={position} id={`position-${position.toLowerCase()}`} />
                        <Label htmlFor={`position-${position.toLowerCase()}`} className="cursor-pointer">
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <Label htmlFor="external" className="cursor-pointer">External URL</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="internal" id="internal" />
                      <Label htmlFor="internal" className="cursor-pointer">Internal Page</Label>
                    </div>
                  </RadioGroup>
                </div>
