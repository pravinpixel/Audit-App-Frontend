...
export default function StatesPage() {
  ...
  useEffect(() => {
    ...
    const mockData: State[] = [
      { id: "1", name: "Maharashtra", districtId: "1", districtName: "Mumbai Suburban", status: "Active" },
      { id: "2", name: "Karnataka", districtId: "3", districtName: "Bangalore Urban", status: "Active" },
      { id: "3", name: "Delhi NCR", districtId: "4", districtName: "Delhi", status: "Active" },
    ]
    setItems(mockData)
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <Button
              onClick={() => navigate("/master/states/add")}
          ...
          <MasterTable
            items={filteredItems}
            columns={["S.NO", "STATE NAME", "DISTRICT", "STATUS", "ACTIONS"]}
            renderRow={(item, index) => [index + 1, item.name, item.districtName, item.status]}
            onEdit={(item) => navigate(`/master/states/edit/${item.id}`)}
