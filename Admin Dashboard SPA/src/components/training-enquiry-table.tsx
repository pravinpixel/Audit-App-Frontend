...
import type { TrainingEnquirySurveyItem } from "@/types/audit-enquiry"
import { useNavigate } from "react-router-dom"

interface TrainingEnquiryTableProps {
  surveys: TrainingEnquirySurveyItem[]
  companyId: string
}

export function TrainingEnquiryTable({ surveys, companyId }: TrainingEnquiryTableProps) {
  const navigate = useNavigate()

  const handleViewEnquiry = (surveyId: string) => {
    navigate(`/audit-enquiry/${companyId}/training-enquiry/${surveyId}`)
  }
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gradient-to-r from-[#E63946] to-[#FF8C00] hover:from-[#E63946] hover:to-[#FF8C00]">
              ...
              <TableHead className="font-semibold text-white">Training Programme Due On</TableHead>
