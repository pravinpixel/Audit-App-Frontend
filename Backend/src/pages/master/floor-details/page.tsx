...
import type { FloorDetail } from "@/types/master"

export default function FloorDetailsPage() {
  const navigate = useNavigate()
  const [searchQuery, setSearchQuery] = useState("")
  const [statusFilter, setStatusFilter] = useState<string>("All")
  const [items, setItems] = useState<FloorDetail[]>([])
  const [filteredItems, setFilteredItems] = useState<FloorDetail[]>([])
  ...
  useEffect(() => {
    ...
    const mockData: FloorDetail[] = [
      ...
      { id: "3", name: "Second Floor", status: "Active" },
      { id: "4", name: "Basement", status: "Active" },
      { id: "5", name: "Mezzanine", status: "InActive" },
    ]
    setItems(mockData)
  ...
  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this floor detail?")) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Dashboard → Floor Details</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <Button
              onClick={() => navigate("/master/floor-details/add")}
          ...
          <MasterTable
            items={filteredItems}
            columns={["S.NO", "FLOOR NAME", "STATUS", "ACTIONS"]}
            renderRow={(item, index) => [index + 1, item.name, item.status]}
            onEdit={(item) => navigate(`/master/floor-details/edit/${item.id}`)}
