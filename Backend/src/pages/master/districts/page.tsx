...
export default function DistrictsPage() {
  ...
  const [filteredItems, setFilteredItems] = useState<District[]>([])
  ...
  useEffect(() => {
    ...
    const mockData: District[] = [
      { id: "1", name: "Mumbai Suburban", status: "Active" },
      { id: "2", name: "Pune", status: "Active" },
      { id: "3", name: "Bangalore Urban", status: "Active" },
      { id: "4", name: "Delhi", status: "Active" },
    ]
    setItems(mockData)
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <Button
              onClick={() => navigate("/master/districts/add")}
          ...
          <MasterTable
            items={filteredItems}
            columns={["S.NO", "DISTRICT NAME", "STATUS", "ACTIONS"]}
            renderRow={(item, index) => [index + 1, item.name, item.status]}
            onEdit={(item) => navigate(`/master/districts/edit/${item.id}`)}
