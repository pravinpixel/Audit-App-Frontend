...
import { ArrowLeft, Building2, MapPin, Phone, Mail, MessageSquare, Send } from "lucide-react"
...
const mockCompanies: Record<string, AuditCompany> = {
  ...
  AUD002: {
    ...
    primaryContact: {
      ...
      email: "anita.desai@manufacturingcorp.com",
    },
  },
  AUD003: {
    id: "AUD003",
    ...
    primaryContact: {
      ...
      email: "vikram.singh@retailenterprises.com",
    },
  },
  AUD004: {
    id: "AUD004",
    ...
    primaryContact: {
      ...
      email: "priya.raman@healthcaresystems.com",
    },
  },
  AUD005: {
    id: "AUD005",
    ...
    primaryContact: {
      ...
      email: "sunil.deshmukh@educationhub.com",
    },
  },
}
...
export default function DataReviewSurveyDetailPage() {
  ...
  const handleBulkApproveExtinguisher = (ids: string[]) => {
    setExtinguisherSurveys((prev) =>
      prev.map((s) => (ids.includes(s.id) ? { ...s, status: "Verified" } : s))
    )
  }
  ...
  const handleRejectExtinguisher = (id: string) => {
    setExtinguisherSurveys((prev) =>
  ...
  const filteredExt = extinguisherSurveys.filter((s) => s.status === "Pending")

  const filteredTrain = trainingSurveys.filter((s) => s.status === "Pending")
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <Button
            variant="ghost"
            onClick={() => navigate("/data-review/survey-data")}
          ...
          <p className="text-sm text-gray-600 mt-1">Review and approve pending survey records</p>
        </div>
        ...
        <Tabs defaultValue="extinguisher" className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="inline-flex items-center text-xs font-medium px-2.5 py-1 rounded-full bg-amber-50 text-amber-600">
              Pending
            </span>
          </div>
